...
export const MusicPlayer = () => {
  ...
  useEffect(() => {
    if (tracks) {
      const backIdx = trackIndex - 1 > 0 ? trackIndex - 1 : tracks.length - 1

      const nextIdx = trackIndex + 1 <= tracks.length - 1 ? trackIndex + 1 : 0

      setTimeout(() => {
        setCovers([tracks[backIdx].cover, tracks[nextIdx].cover])

        setStrings([tracks[backIdx].title, tracks[nextIdx].title])
      }, 500)
    }
  }, [trackIndex, tracks])
  ...
  useEffect(() => {
    if (tracks) {
      setCurrentTime(0)
      setCurrentTrack(tracks[trackIndex].audio)
    }
  }, [trackIndex, tracks])
  ...
  function next() {
    if (!shuffleMode) {
      ...
      setTrackIndex(getRandomIntInclusive(0, tracks.length - 1))
      setAudioPlay(true)
    }
  }
  ...
  function prev() {
    ...
    setTrackIndex(trackIndex === 0 ? tracks.length - 1 : trackIndex - 1)
        ...
        <>
          <div className={styles.hidden}>
            <AudioPlayer
              ...
              listenInterval={10}
              src={getMediaPath(
                tracks[trackIndex].attributes.audio.data.attributes
              )}
      ...
      ></div> */}
          <div className={styles.wrapper}>
            <div className={styles.info}>
              <Cubes
                hovered={false}
                string={tracks[trackIndex].title}
                cover={[
                  getMediaPath(
                    tracks[trackIndex].attributes.cover.data.attributes
                  ),
                  getMediaPath(
                    tracks[trackIndex].attributes.cover.data.attributes
                  ),
              ...
              <div className={styles.text}>
                <p className={styles.title}>{tracks[trackIndex].title}</p>
                <span className={styles.artist}>
                  {Array.isArray(tracks[trackIndex].artists)
                    ? Array.from(tracks[trackIndex].artists).join(', ')
